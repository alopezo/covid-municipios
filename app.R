...
ui <- fluidPage(
  ...
  fluidRow(column(12,h2("Casos y muertes")),br()),
  fluidRow(
    column(12, align="center",
  ...
  br(),
  hr(),
  fluidRow(column(12,h2("Vacunación"),
                  br())),
  fluidRow( column(2, pickerInput(
    inputId="poblacion",
    label = "Población",
    choices = list("Población de 60 años y más"=">60",
                   "Trabajadores/as de la Salud"="Salud",
                   "Personal estratégico"="Estratégico",
                   "Población de riesgo"="Riesgo",
                   "Otros"="Otros"),
    selected = list("Población de 60 años y más"=">60",
                    "Trabajadores/as de la Salud"="Salud",
                    "Personal estratégico"="Estratégico",
                    "Población de riesgo"="Riesgo",
                    "Otros"="Otros"),
    ...
    pickerInput(
      inputId="vacuna",
      label = "Vacuna",
      choices = unique(vacunasFecha$vacuna),
      selected = unique(vacunasFecha$vacuna),
      multiple = TRUE),
    pickerInput(
      inputId="orden_dosis",
      label = "Dosis",
      choices = list("Primera dosis"=1,
                     "Segunda dosis"=2),
      selected = list("Primera dosis"=1,
                      "Segunda dosis"=2),
      multiple = TRUE)),
...
server <- function(input, output, session) {
  ...
  output$grafico1 <-renderDygraph({
    ...
    dg <- dygraph(data=data, main = paste0(titulo,' - ',input$select_depto)) %>% dySeries(name=input$select_depto, label=input$select_depto, color = 'black', strokeWidth = 1.8) %>% dyRangeSelector()
    if (exists("series")==T)
    {parte_dg2 <- paste(series, collapse= " %>% ")
    }
    if (exists("series")==T)
    {
      eval(parse(text=paste0("dg %>% ",parte_dg2, " %>% dyRangeSelector()")))
    }
    else {dg}
  })
  ...
  output$graf_vacunas <- renderDygraph({
    if (input$acum=="Diario" | input$acum=="Acumulado")
    {
      vac <- vacunasFecha %>% filter(depto_residencia == input$select_depto &
                                       condicion_aplicacion %in% input$poblacion &
                                       vacuna %in% input$vacuna &
                                       orden_dosis %in% input$orden_dosis) %>%
        group_by(fecha_aplicacion) %>%
        dplyr::summarise(vacunas=sum(vacunas))
      
      vac$fecha_aplicacion <- as.Date(vac$fecha_aplicacion)
      fecha <- seq(as.Date('2020-12-29'),as.Date(max(vac$fecha_aplicacion)),by=1)
      vac <- left_join(data.frame(fecha_aplicacion=fecha),vac)
      
      vac$vacunas[is.na(vac$vacunas)] <- 0
      
      
      vac <- if (input$acum=="Acumulado") {vac %>% mutate(vacunas=cumsum(vacunas))} else {vac}
      
      vac <- xts(x=vac$vacunas, order.by = as.Date(vac$fecha_aplicacion))  
      colnames(vac)[1] <- "Vacunas aplicadas"
      titulo <- if (input$acum=="Diario") {"Aplicaciones diarias "} else {"Aplicaciones acumuladas "}
      dygraph(vac, main = paste0(titulo,"- ", input$select_depto)) %>% dyRangeSelector() 
      ...
      
    {
      ...
      vac <- data.frame(i=seq(1,nrow(vac),1),vacunas=vac$vacunas,SE=vac$SE)
      vac$vacunas <- as.numeric(vac$vacunas)
      
      
      colnames(vac)[2] <- "Vacunas aplicadas"
      

      valueFormatter <- "function formatValue(v) {
                           if (v == 1) return 53 + ' de 2020';
      ...
      dygraph(vac, main = paste0(titulo)) %>% 
        dyAxis("x", label = "Semana epi", axisLabelFormatter=JS(valueFormatter), pixelsPerLabel = 40, valueRange=c(1,100)) %>% 
        dyRangeSelector(dateWindow = c(1, max(vac[,1]+1))) 
